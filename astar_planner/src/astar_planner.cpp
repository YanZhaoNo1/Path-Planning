...
namespace astar_planner{
    ...
    /*
    获取地图的高度（height）和宽度（width），将地图栅格化costmap->getCost(j, i)
    获取每个cell的cost_value,当costvalue=0，则此cell为自由可通行区域，
    否则该cell被占用，地图中所有cells是否free都存储在OGM[map_size]
    */
    ...
    /*
    makePlan继承nav_core::BaseGlobalPlanner ，发布plan
    */
...
/*导出astar_planner::AstarPlanner class为ROS做插件 */
//register this planner as a BaseGlobalPlanner plugin
PLUGINLIB_EXPORT_CLASS(astar_planner::AstarPlanner, nav_core::BaseGlobalPlanner)
