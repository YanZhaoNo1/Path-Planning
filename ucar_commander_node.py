...
class UcarCommander(object):
    ...
    def __init__(self) -> None:
        ...
        self.scan_goal.target_pose.pose.position.x = 2.348
        self.scan_goal.target_pose.pose.position.y = -3.103
    ...
    def _scan_qr_code(self):
        ...
        if id == -1:
            ...
            # 根据扫码结果设置终点目标
            if id == 0:
                #Frame:map, Position(-0.380, -1.348, 0.000), Orientation(0.000, 0.000, 0.708, 0.706) = Angle: 1.57
                self.scan_goal.target_pose.header.frame_id = self.global_frame_id
                self.scan_goal.target_pose.pose.position.x = -0.380
                self.scan_goal.target_pose.pose.position.y = -1.348
                self.scan_goal.target_pose.pose.position.z = 0
                self.scan_goal.target_pose.pose.orientation.x = 0
                self.scan_goal.target_pose.pose.orientation.y = 0
                self.scan_goal.target_pose.pose.orientation.z = 0.708
                self.scan_goal.target_pose.pose.orientation.w = 0.706
